...
if [ $# -ne 3 ]
then
  echo "Usage: $0 <environment_name> <region> <stack_name>"
  exit
fi
...
#   echo "ERROR: Stack $STACKNAME in Environment/Region $ENVNAME/$REGION already exists!"
#   exit 1
# fi
...
# And do what we need
echo "Creating stack in $ENVNAME/$REGION: $STACKNAME"

# Create stack folder
echo "  Creating folder: $ENVNAME/$REGION/$STACKNAME"
echo "  Entering directory: $ENVNAME/$REGION/$STACKNAME"
mkdir "$ENVNAME/$REGION/$STACKNAME"
cd "$ENVNAME/$REGION/$STACKNAME"
...
if [ -d "$OLDPWD/templates/$STACKNAME" ]; then
  echo "Template $STACKNAME found, setting up template..."
  for TEMPLATE_FILE in $(ls ../../../templates/$STACKNAME | tr '/' ' '); do
    echo "Symlinking template: $TEMPLATE_FILE"
    ln -s ../../../templates/$STACKNAME/$TEMPLATE_FILE
  done
  echo "Completed setting up template for $STACKNAME"
fi
...
echo ""
echo "Creation complete: $ENVNAME/$REGION/$STACKNAME"
echo ""
